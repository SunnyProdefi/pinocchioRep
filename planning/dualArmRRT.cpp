...
bool isStateValid(const ob::State *state) {
  ...
  bool hasCollision =
      CollisionCheck(model, data, geom_model, geom_data, q_total, true);
...
void planWithRRT(const Eigen::VectorXd &q_init, const Eigen::VectorXd &q_goal,
  ...
  for (size_t i = 0; i < 12; ++i) {
    std::cout << "Dimension " << i + 1 << ": Low = " << bounds.low[i]
              << ", High = " << bounds.high[i] << std::endl;
  }
  ...
  if (solved) {
    ...
    if (geoPath) {
      ...
      std::size_t stateDimension = geoPath->getSpaceInformation()
                                       ->getStateSpace()
      ...
      for (std::size_t i = 0; i < pathLength; ++i) {
        const auto *state =
            geoPath->getState(i)->as<ob::RealVectorStateSpace::StateType>();
...
void setQGoal(Eigen::VectorXd &q_goal, const Eigen::VectorXd &angles_degrees,
  ...
  for (size_t i = 0; i < 12; ++i) {
    ...
    if (rad_angle < lower_limits[i] || rad_angle > upper_limits[i]) {
      std::cout << "Error: Angle for joint " << i
                << " is out of bounds: " << angles_degrees[i]
...
int main(int argc, char **argv) {
  ...
  std::cout << "Selected joint lower limits: " << lower_limits.transpose()
            << std::endl;
  std::cout << "Selected joint upper limits: " << upper_limits.transpose()
            << std::endl;
  ...
  // 生成插值点
  std::vector<Eigen::VectorXd> interpolated_points =
      interpolator.generatePoints(pathPoints, pathPoints.cols() * 5);
  // 打印插值点
